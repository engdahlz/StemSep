...
def extract_freqs_from_checkpoint(ckpt_path):
    ...
    if not freqs_per_bands:
        print("  ⚠️ Could not find band_split.to_features - might be different architecture")
        return None
...
def update_yaml_with_config(yaml_path, extracted_config):
    ...
    if "mask_estimator_depth" in extracted_config:
        ...
        if old != extracted_config["mask_estimator_depth"]:
            updates.append(f"mask_estimator_depth: {old} → {extracted_config['mask_estimator_depth']}")
    ...
    if "skip_final_norm" in extracted_config:
        ...
        if old != extracted_config["skip_final_norm"]:
            updates.append(f"skip_final_norm: {old} → {extracted_config['skip_final_norm']}")
    
    if "freqs_per_bands" in extracted_config:
        model_config["freqs_per_bands"] = tuple(extracted_config["freqs_per_bands"])
        updates.append(f"freqs_per_bands: {len(extracted_config['freqs_per_bands'])} bands")
...
def main():
    ...
    if fixed_count > 0:
        print("\nRun diagnose_model.py again to verify all models now have 100% match!")
