...
# ============================================================================
from audio.roformer import BSRoformer, MelBandRoformer
...
class RoformerSeparator:
    ...
    def _detect_model_type(self, model_cfg: Dict[str, Any]) -> str:
        ...
        if 'freqs_per_bands' in model_cfg:
            ...
            raise ValueError("Cannot detect model type: missing freqs_per_bands or num_bands")
    ...
    def _create_bs_roformer(self, model_cfg: Dict[str, Any]):
        ...
        for key in ['stft_n_fft', 'stft_hop_length', 'stft_win_length', 'mask_estimator_depth',
                    'dim_freqs_in', 'stft_normalized', 'skip_final_norm', 'skip_connection',
                    'mlp_expansion_factor', 'sage_attention', 'zero_dc', 'use_torch_checkpoint']:
            if key in model_cfg:
                model_args[key] = model_cfg[key]
        
        self.logger.info(f"Creating BSRoformer with skip_final_norm={model_args.get('skip_final_norm', False)}")
    ...
    def _create_mel_band_roformer(self, model_cfg: Dict[str, Any]):
        ...
        # Optional params including skip_final_norm (now supported!)
        for key in ['sample_rate', 'stft_n_fft', 'stft_hop_length', 'stft_win_length', 
                    'stft_normalized', 'mask_estimator_depth', 'skip_final_norm', 'skip_connection',
                    'mlp_expansion_factor', 'sage_attention', 'zero_dc', 'use_torch_checkpoint']:
            if key in model_cfg:
                model_args[key] = model_cfg[key]
        
        self.logger.info(f"Creating MelBandRoformer with skip_final_norm={model_args.get('skip_final_norm', False)}")
    ...
    def load_model(self, model_id: str) -> Tuple[nn.Module, Dict, str]:
        ...
        if result.missing_keys:
            self.logger.warning(f"Missing keys when loading weights: {len(result.missing_keys)} keys")
            self.logger.debug(f"Missing keys: {result.missing_keys[:5]}...")
        if result.unexpected_keys:
            self.logger.warning(f"Unexpected keys in checkpoint: {len(result.unexpected_keys)} keys")
            self.logger.info(f"Unexpected keys (first 10): {result.unexpected_keys[:10]}")
    ...
    def separate(
        ...
        if model_cfg.get('stereo', True) and mix.shape[0] == 1:
        ...
        chunk_size = audio_cfg.get('chunk_size', inference_cfg.get('chunk_size', 485100))
    ...
    def _demix(self, model, mix: np.ndarray, chunk_size: int, 
               num_overlap: int, batch_size: int) -> np.ndarray:
        """
