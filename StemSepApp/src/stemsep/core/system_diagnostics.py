...
class SystemDiagnostics:
    ...
    def check_cuda_availability(self) -> Dict[str, Any]:
        ...
        result = {"available": False, "gpu_count": 0, "cuda_version": None, "gpus": []}
                ...
                for i in range(result["gpu_count"]):
                    gpu_info = {
                        ...
                        "total_memory_gb": round(
                            self._torch.cuda.get_device_properties(i).total_memory
                            / (1024**3),
                            2,
                        ),
                    }
    ...
    def can_run_model(self, required_vram_gb: float) -> Dict[str, Any]:
        ...
        logger.debug(
            f"Checking if system can run model requiring {required_vram_gb} GB VRAM"
        )
    ...
    def get_recommended_gpu_profile(self) -> Dict[str, Any]:
        ...
        try:
            # Preferred import after namespace refactor
            from stemsep.models.model_config import GPU_PROFILES, get_gpu_settings
        except ImportError:
            try:
        ...
        if cuda_info["available"] and cuda_info["gpus"]:
            ...
            try:
                ...
                if detector_info.get("gpus"):
                    ...
                    logger.info(
                        f"Using GPUDetector fallback: {gpu_info['name']} with {vram_gb} GB"
                    )
            except Exception as e:
...
if __name__ == "__main__":
    ...
    print(
        f"RAM: {report['memory']['available_gb']:.1f} GB available / {report['memory']['total_gb']:.1f} GB total"
    )
