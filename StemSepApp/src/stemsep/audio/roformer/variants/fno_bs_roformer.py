...
from stemsep.audio.roformer.attend import Attend

try:
    from stemsep.audio.roformer.attend_sage import Attend as AttendSage  # type: ignore
except Exception:
    AttendSage = None
...
class Attention(Module):
    def __init__(
        ...
        if sage_attention and AttendSage is not None:
...
class MaskEstimator(Module):
    ...
    def __init__(self, dim, dim_inputs: Tuple[int, ...], depth, mlp_expansion_factor=4):
        ...
        if FNO1d is None:
            raise ImportError(
                "FNO models require the 'neuraloperator' (neuralop) dependency. "
                "Install it and restart StemSep. Original import error: "
                + repr(_FNO_IMPORT_ERROR)
            )
        self.dim_inputs = dim_inputs
