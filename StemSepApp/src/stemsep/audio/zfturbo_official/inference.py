...
# Support configs that contain !!python/tuple tags (common in MSST YAMLs)
...
class ZFTurboInference:
    ...
    def load_config(self, model_id: str) -> dict:
        ...
        with open(yaml_path, "r", encoding="utf-8") as f:
            # Prefer safe_load; we also allow !!python/tuple via custom constructor above.
            config = yaml.safe_load(f)

        return config

    def _find_checkpoint_path(self, model_id: str) -> Path:
        """Find a local checkpoint file for a model.

        MSST models use various extensions (.ckpt, .chpt, .pth, ...).
        ModelManager may alias URL basenames to `<model_id>.<ext>`.
        """
        for ext in (".ckpt", ".chpt", ".pth", ".pt", ".safetensors"):
            p = self.models_dir / f"{model_id}{ext}"
            if p.exists():
                return p
        raise FileNotFoundError(
            f"Checkpoint not found for '{model_id}' in {self.models_dir} (tried .ckpt/.chpt/.pth/.pt/.safetensors)"
        )
    ...
    def load_model(self, model_id: str, config: dict) -> nn.Module:
        ...
        elif model_type in ["bandit", "BandIt"]:
            from stemsep.models.model_factory import ModelFactory

            model = ModelFactory.create_model("BandIt", config.get("model", config))
        elif model_type in ["apollo", "Apollo"]:
            from stemsep.models.model_factory import ModelFactory

            model = ModelFactory.create_model("Apollo", config.get("model", config))
        else:
        ...
        # Load checkpoint
        ckpt_path = self._find_checkpoint_path(model_id)
        ...
        if result.unexpected_keys:
            self.logger.warning(
                f"Unexpected keys: {len(result.unexpected_keys)} (expected for some models)"
            )
    ...
    def demix(
        self, config: dict, model: nn.Module, mix: np.ndarray, pbar: bool = False
    ) -> Dict[str, np.ndarray]:
        """
        ...
        from stemsep.audio.zfturbo_demix import demix as official_demix
    ...
    async def separate(
        ...
        self.logger.info(
            f"Output waveforms after mapping: {list(output_waveforms.keys())}"
        )
    ...
    def _apply_tta(
        self, config: dict, model: nn.Module, mix: np.ndarray, waveforms_orig: dict
    ) -> dict:
        ...
        for i, aug_mix in enumerate(augmentations):
            ...
            for stem in waveforms:
                if i == 0:
                    waveforms_orig[stem] = (
                        waveforms_orig.get(stem, 0) + waveforms[stem][::-1].copy()
                    )
                else:
...
async def test():
    ...
    parser.add_argument(
        "model_id", nargs="?", default="unwa-inst-v1e-plus", help="Model ID"
    )
    parser.add_argument(
        "output_dir", nargs="?", default="test_output", help="Output directory"
    )
