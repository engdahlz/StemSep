...
class EnsembleBlender:
    ...
    def get_stem_algorithm_preset(cls, preset: str = "balanced_sdr") -> Dict:
        ...
        """
        return cls.STEM_ALGORITHM_PRESETS.get(
            preset, cls.STEM_ALGORITHM_PRESETS["balanced_sdr"]
        )
    ...
    def get_phase_fixer_settings(cls, preset: str = "roformer_buzzing") -> Dict:
        ...
        """
        return cls.PHASE_FIXER_PRESETS.get(
            preset, cls.PHASE_FIXER_PRESETS["roformer_buzzing"]
        )
    ...
    def phase_fix_workflow(
        ...
        """
        self.logger.info(
            f"Phase Fix Workflow: {low_hz}-{high_hz}Hz, weight={high_freq_weight}"
        )
        ...
        # Ensure shapes match (channels and length)
        min_ch = min(phase_fixed.shape[0], reference_instrumental.shape[0])
        phase_fixed = phase_fixed[:min_ch, :]
        reference_instrumental = reference_instrumental[:min_ch, :]

        min_len = min(phase_fixed.shape[-1], reference_instrumental.shape[-1])
        phase_fixed = phase_fixed[..., :min_len]
        reference_instrumental = reference_instrumental[..., :min_len]
    ...
    def blend_stems(
        ...
        algorithm: Literal[
            "average", "max_spec", "min_spec", "phase_fix", "frequency_split"
        ] = "average",
        **kwargs,
    ) -> Dict[str, np.ndarray]:
        """
        ...
        for stem_name, audio_list in stem_map.items():
            ...
            for i in range(len(audio_list)):
                if (
                    audio_list[i].ndim == 2
                    and audio_list[i].shape[0] > audio_list[i].shape[1]
                ):
            ...
            if algorithm == "average":
                ...
                if TORCH_AVAILABLE:
                    ...
                    self.logger.warning(
                        "Torch not available for Max Spec, falling back to Average"
                    )
                    blended = self._average_blend(audio_list, weights)
                ...
                if TORCH_AVAILABLE:
                    ...
                    self.logger.warning(
                        "Torch not available for Min Spec, falling back to Average"
                    )
                    blended = self._average_blend(audio_list, weights)
                ...
                if TORCH_AVAILABLE:
                    if len(audio_list) < 2:
                        self.logger.warning(
                            "Phase Fix requires at least 2 sources (Magnitude, Phase Ref). Falling back to Average."
                        )
                        blended = self._average_blend(audio_list, weights)
                    else:
                    ...
                    self.logger.warning(
                        "Torch not available for Phase Fix, falling back to Average"
                    )
                    blended = self._average_blend(audio_list, weights)
                ...
                if TORCH_AVAILABLE:
                    ...
                    self.logger.warning(
                        "Torch not available for Frequency Split, falling back to Average"
                    )
                    blended = self._average_blend(audio_list, weights)

            else:
                self.logger.warning(
                    f"Unknown algorithm: {algorithm}, falling back to Average"
                )
                blended = self._average_blend(audio_list, weights)
    ...
    def blend_stems_mixed(
        ...
        **kwargs,
        ...
        for stem_name, audio_list in stem_map.items():
            ...
            stem_weights = (
                weights if weights and len(weights) == len(audio_list) else None
            )

            # Use existing blend_stems for single stem
            single_stem_map = {stem_name: audio_list}
            result = self.blend_stems(
                single_stem_map, stem_weights, algorithm, **kwargs
            )
    ...
    def validate_ensemble_size(self, num_models: int) -> Dict:
        ...
        if num_models > max_recommended:
            result["warning"] = (
                f"Using {num_models} models may degrade quality. Max recommended: {max_recommended}"
            )
    ...
    def get_recommended_algorithm(
        self, target_stem: str, priority: str = "balanced"
    ) -> str:
        """
    ...
    def _average_blend(
        self, audio_list: List[np.ndarray], weights: Optional[List[float]]
    ) -> np.ndarray:
    ...
    def _frequency_split_blend(
        self, audio_list: List[np.ndarray], split_freq: int = 750
    ) -> np.ndarray:
        """
        ...
        if len(audio_list) != 2:
            self.logger.warning(
                f"Frequency Split requires exactly 2 models, got {len(audio_list)}. Fallback to average."
            )
        ...
        for c in range(channels):
            ...
            if end_bin > start_bin:
                ...
                ramp = torch.linspace(
                    1.0, 0.0, end_bin - start_bin, device=device
                ).unsqueeze(1)
    ...
    def _phase_fix_blend(
        ...
        for c in range(channels):
            ...
            # Weight semantics:
            # - 0.0 => keep original phase (no change)
            # - 1.0 => fully replace with reference phase in the selected band
            # - >1.0 => aggressive replacement. Since direct phase interpolation should not exceed 1.0,
            #          interpret values >1.0 as "full replace" but keep the caller-visible value for logging.
            blend_factor = float(high_freq_weight)
            if blend_factor < 0.0:
                blend_factor = 0.0
            if blend_factor > 1.0:
                blend_factor = 1.0
            ...
            blended_angle[low_bin:high_bin, :] = (1.0 - blend_factor) * angle_mag[
                low_bin:high_bin, :
    ...
    def demudder_combine(
        ...
        self.logger.info(
            f"Demudder combine: weights=({w_orig:.2f}, {w_rotate:.2f}, {w_remix:.2f})"
        )
        return combined

    def _apply_high_pass(
        self, audio: np.ndarray, cutoff_hz: int, sr: int = 44100
    ) -> np.ndarray:
