...
interface SeparatePageProps {
  ...
  onNavigateToConfigure?: (fileName: string, filePath: string, presetId?: string) => void
  onNavigateToResults?: () => void
  pendingSeparationConfig?: PendingSeparationConfig | null
  onClearPendingConfig?: () => void
}

export default function SeparatePage({ onNavigateToModels, onNavigateToConfigure, onNavigateToResults, pendingSeparationConfig, onClearPendingConfig }: SeparatePageProps) {
  // --- State ---
  ...
  const [selectedPreset, setSelectedPreset] = useState<string>('best_instrumental')
  const [showPresetBrowser, setShowPresetBrowser] = useState(false)
  const [favoritePresetIds, setFavoritePresetIds] = useState<string[]>(() => {
    const saved = localStorage.getItem('favoritePresets')
    return saved ? JSON.parse(saved) : ['best_instrumental', 'best_vocals', 'best_karaoke']
  })
  const [presetAvailability, setPresetAvailability] = useState<Record<string, {
    available: boolean
    model_id: string
    ...
    const loadGpuInfo = async () => {
      if (window.electronAPI?.getGpuDevices) {
        try {
          ...
          const vram = info?.recommended_profile?.vram_gb || info?.gpus?.[0]?.memory_gb || 0
          setGpuVRAM(vram)
        } catch (e) {
  ...
  useEffect(() => {
    ...
    if (window.electronAPI?.onBridgeReady) {
      cleanup = window.electronAPI.onBridgeReady((data) => {
        console.log(`Bridge ready! Models: ${data.modelsCount}, Recipes: ${data.recipesCount}`)
  ...
  const combinedPresets: Preset[] = useMemo(() => {
    ...
    const recipePresets = recipes.map(r => ({
      ...
      stems: r.target ? [r.target] : [], // Ensure stems is string[]
      recommended: false, // Required by Preset interface
      category: 'smart',
      modelId: undefined, // Optional
      ensembleConfig: {
      ...
      isRecipe: true // Custom property, might be ignored by Preset type but useful
    })) as unknown as Preset[] // Force cast to avoid minor mismatches if any
  ...
  const [_storedConfig, setStoredConfig] = useState<SeparationConfig | null>(null)
  ...
  useEffect(() => {
    const removeProgressListener = window.electronAPI?.onSeparationProgress((data) => {
      setSeparationProgress(data.progress, data.message)
      // Update the currently processing queue item with message for phase detection
    ...
    const removeCompleteListener = window.electronAPI?.onSeparationComplete((data) => {
    ...
    const removeErrorListener = window.electronAPI?.onSeparationError((data) => {
  ...
  }, [setSeparationProgress, completeSeparation, addLog, queue, updateQueueItem])
  ...
  const addToQueue = useCallback(async (paths: string[]) => {
    ...
    toast.success(`Added ${paths.length} file${paths.length > 1 ? 's' : ''} to queue`)
  ...
  const startSeparationWithFilePath = async (config: SeparationConfig, filePath: string) => {
    ...
    // Determine model and stems from config
    let modelId = config.modelId || 'htdemucs'
    let stems = config.stems

    // If simple mode and preset selected, use preset config
    if (config.mode === 'simple' && config.presetId) {
    ...
    // If ensemble config is present, always use 'ensemble' as modelId
    // and ensure stems are set (default to vocals+instrumental)
    ...
    try {
      ...
      const presetInfo = config.presetId
      ...
      addToHistory({
        ...
        modelId: modelId,
        modelName: presetInfo?.name || models.find(m => m.id === modelId)?.name || modelId,
        preset: presetInfo ? { id: presetInfo.id, name: presetInfo.name } : undefined,
        status: 'completed',
  ...
  const handleResolveYouTube = useCallback(async () => {
    ...
    try {
      ...
      const displayName = title || resolvedPath.split(/[\\/]/).pop() || 'YouTube Audio'
  ...
  const currentPreset = combinedPresets.find(p => p.id === selectedPreset)
  ...
  return (
    <div className="h-full flex flex-col bg-background text-foreground selection:bg-primary/30" onDragEnter={handleWindowDragEnter}>
      ...
      <div className="flex-1 overflow-y-auto relative scroll-smooth">
        ...
        <div className="flex flex-col max-w-4xl mx-auto w-full p-6 gap-8 relative z-10 animate-in fade-in duration-700">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold tracking-tight">New Separation</h1>
              <p className="text-muted-foreground mt-1">Drop your audio files to begin.</p>
            </div>
          ...
          <div className="flex-1 min-h-[400px] flex flex-col">
            ...
            >
              ...
              {/* Background Glow */}
              <div className={`absolute inset-0 bg-gradient-to-br from-primary/5 via-transparent to-primary/5 opacity-0 transition-opacity duration-500 pointer-events-none ${isDragging ? 'opacity-100' : 'group-hover:opacity-50'}`} />
              ...
              <CardContent className="flex-1 flex flex-col items-center justify-center text-center space-y-8 relative z-10 pointer-events-none py-20">
                <div className={`p-8 rounded-full bg-background/80 backdrop-blur-sm shadow-xl transition-all duration-500 group-hover:scale-110 group-hover:shadow-2xl group-hover:shadow-primary/20 ${isDragging ? 'scale-125 bg-primary/20 text-primary' : 'text-muted-foreground'}`}>
                  <Upload className={`w-16 h-16 transition-all duration-500 ${isDragging ? 'animate-bounce' : ''}`} />
                </div>
                <div className="space-y-4 max-w-md mx-auto">
                  <h3 className="text-3xl font-bold tracking-tight">
                    {isDragging ? 'Drop files to separate' : 'Drop audio files here'}
                  </h3>
                  <p className="text-muted-foreground text-lg">
                    or click anywhere to browse
                  </p>
                  <div className="flex flex-wrap justify-center gap-2 pt-4">
                    <Badge variant="secondary" className="bg-background/50 backdrop-blur-sm border-border/50 text-xs px-3 py-1">MP3</Badge>
                    <Badge variant="secondary" className="bg-background/50 backdrop-blur-sm border-border/50 text-xs px-3 py-1">WAV</Badge>
                    <Badge variant="secondary" className="bg-background/50 backdrop-blur-sm border-border/50 text-xs px-3 py-1">FLAC</Badge>
                    <Badge variant="secondary" className="bg-background/50 backdrop-blur-sm border-border/50 text-xs px-3 py-1">M4A</Badge>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="flex flex-col gap-3">
              ...
              {(youtubeStatus || youtubePercent) && (
                <div className="text-xs text-muted-foreground">
                  {youtubeStatus}{youtubePercent ? ` - ${youtubePercent}` : ''}
                </div>
              )}
              <div className="text-xs text-muted-foreground">
                Tip: The audio is downloaded to a temporary WAV file and then treated like a normal local file.
              </div>
            </div>
          </Card>
                  ...
                  {isProcessing ? (
                    <span className="flex items-center gap-2 text-primary font-medium">
                      ...
                      <span className="flex flex-col">
                        <span>Processing {queue.filter(i => i.status === 'completed').length} / {queue.length} files...</span>
                        {progressMessage && <span className="text-xs text-muted-foreground">{progressMessage}</span>}
                      </span>
                    </span>
                  ) : (
                    <span>Ready to process <b>{queue.length}</b> files with <b>{currentPreset?.name}</b></span>
                  )}
                </div>
                {isProcessing ? (
                  <Button
